...
foreach ($user in $users) {
    ...
    try {
        ...
        Write-Log "✅ Password for $username has been reset on $(Get-Date -Format 'dd-MM-yyyy HH:mm:ss'). User must change it at next login."
    }
    catch {
	# Handle any errors that occur during password reset
	Write-Log "❌ Failed to reset password for $username - $($_.Exception.Message)"
    }
}